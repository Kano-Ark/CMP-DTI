...
def data_preparation(data, task):
    ...
    with open('dataset/' + task + '/normalized_seq_esm.csv') as f2:
    ...
    for i in tqdm(data):
        t = i.split(',')
        drug_id = t[0]
        if os.path.exists('dataset/' + task + '/Drug_Point_Graph/' + str(t[0]) + '.txt') and os.path.exists('dataset/' + task + '/protein_image_3D_normal/' + str(t[1]) + '.txt')and drug_id in drug_list:
...
def data_preparation_protein_all(task):
    ...
    protein_info = np.loadtxt('dataset/' + task + '/normalized_seq_esm.csv', delimiter=',', dtype=str)
    batch_bert = []
    batch_point = []
    for i in trange(idx.shape[0]):
    ...
    batch_bert = torch.from_numpy(np.array(batch_bert, dtype=np.float32)).float()
    batch_point = torch.from_numpy(np.array(batch_point, dtype=np.float32)).float()
...
def data_preparation_protein_graph_all(task):
    print('protein graph data preparation....')
    graph_dir = os.path.join('dataset', task, 'protein_graphs')
    csv_path = os.path.join('dataset', task, 'normalized_seq_esm.csv')

    protein_info = np.loadtxt(csv_path, delimiter=',', dtype=str)
    protein_names = [row[0] for row in protein_info]

    graph_data = []
    for name in tqdm(protein_names):
        graph_path = os.path.join(graph_dir, f"{name}.pt")
        if os.path.exists(graph_path):
            xg = torch.load(graph_path)  # 期望: (n, 320+n)
        else:
            # 缺失时：先给一个最小空图占位 (1, 321) 或者按你希望的方式
            # 这里给 (1, 321) 避免后续 d 推断崩掉
            xg = torch.zeros((1, 321))
            xg[0, 0] = 1.0         # X[0,0]=1
            xg[0, 320] = 1.0       # A[0,0]=1
        graph_data.append(xg)
